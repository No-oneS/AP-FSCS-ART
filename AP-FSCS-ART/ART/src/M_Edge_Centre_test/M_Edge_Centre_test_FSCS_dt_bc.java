...
public class M_Edge_Centre_test_FSCS_dt_bc{
...
//	el2 real = new el2();			// 初始化真实程序类
	...
	public M_Edge_Centre_test_FSCS_dt_bc(double[][]  bd, double failrate, int fail_number){
...
//		fzb = new FaultZone_Strip(bd, (float)failrate, fzb_rate);
		...
		Edge_Centre = new M_Edge_Centre(dimlist);			//初始化M_Edge_Centre类
//		bound_center_rate = 8;
...
//        	System.out.println("bound_num ="+bound_num);
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int[] temp=new M_Edge_Centre_test_FSCS_dt_bc(list,fail_rate,fail_number).run();;
...
//		int times =  1;
		...
		double failrate= real_Parameters.M_Edge_Centre_4D_rate;
...
//		}
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback(real_Parameters.M_Edge_Centre_4D, failrate,fail_number));	
		}
...
//				System.out.println("future.take() = " + future.get() );
		...
		System.out.println("M_Edge_Centre_test_FSCS_dt_bc failrate ： "+ failrate +" ,fail_number "+fail_number+"\n且最后的AVE(M(Edge:Centre)): "+sums / (double) times);
		System.out.println("Time : "+(endTime - startTime) / (double)times );
		
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/M_Edge_Centre_test/M_Edge_Centre_test_FSCS_dt_bc_1000_M_Edge_Centre_4D.txt");
		try {
