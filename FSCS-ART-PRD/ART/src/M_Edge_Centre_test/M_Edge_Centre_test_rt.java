...
public class M_Edge_Centre_test_rt {
...
//	double band_dist_max = 0;				// 点到边界的最大值
	...
	public M_Edge_Centre_test_rt(double[][]  bd, double failrate, int fail_number){
...
//			mindist = mindist > band_dist ? band_dist : mindist;	
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int[] temp=new M_Edge_Centre_test_rt(list,fail_rate,fail_number).run();;
			return temp;
		}
	}
...
//				System.out.println("future.take() = " + future.get() );
		...
		System.out.println("M_Edge_Centre_test_rt failrate ： "+ failrate +" ,fail_number "+fail_number+"\n且最后的AVE(M(Edge:Centre)): "+sums / (double) times);
		System.out.println("Time : "+(endTime - startTime) / (double)times );
	
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/M_Edge_Centre_test/M_Edge_Centre_test_rt_1000_M_Edge_Centre_4D.txt");
		try {
