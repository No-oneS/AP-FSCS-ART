...
//	main m = new main();
	...
	public real_program5(ArrayList<Dimension> dim_list) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//		return false;
	...
	public static void main(String args[]) {
		...
		for (int i = 1; i <= times; i++) {
			...
			temp = new real_program5(list).run();
...
//			e.printStackTrace();
//		}
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/real_software/FSCS_NNDC_ART_real_data/FSCS_NNDC_ART_real_el2.txt");
		try {
			FileWriter stream = new FileWriter(file2);
			// stream.write("sdfdsfdsfds");
			stream.write("real_Parameters :  el2 \t failrate:  "+ failrate+"\t Fm: " + sums / (double) times + "  且最后的Fart/Frt: " + sums / (double) times * failrate+"\t\t   Time :  "+(total) / (double) times+"\r\n\n");
