...
public class Simulation_square_7 {
...
//	main m = new main();
	...
	double fail_length;
	public Simulation_square_7(ArrayList<Dimension> dim_list, double failrate) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//		list.add(u);
		...
		for (int i = 1; i <= times; i++) {
			temp=new Simulation_square_7(list, failrate).run();
			result.add(temp);
...
//		for (int i=1; i <= times; i++) {
//			 cService.submit(new ThreadWithCallback1(list, failrate));
//		}
//		try {
//			for(int i=1;i<=times;i++){
...
class ThreadWithCallback1 implements Callable{
	...
	ThreadWithCallback1(ArrayList<Dimension> list,double fail_rate){
		this.list=list;
	...
	public Object call() throws Exception {
		...
		int temp=new Simulation_square_7(list,fail_rate).run();
		return temp;
	}
