...
public class Simulation_Mutiply_Main_7 {
	...
	int fail_number;
	double w;
	public Simulation_Mutiply_Main_7(ArrayList<Dimension> dim_list, double failrate, int a,double w) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//		}
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback4(list, failrate));
		}
		try {
			for(int i=1;i<=times;i++){
...
class ThreadWithCallback4 implements Callable{
	private ArrayList<Dimension> list;
	private double fail_rate;
	ThreadWithCallback4(ArrayList<Dimension> list,double fail_rate){
		this.list=list;
	...
	public Object call() throws Exception {
		...
		int temp=new Simulation_Mutiply_Main_7(list,fail_rate,70,0.3).run();
		return temp;
	}
