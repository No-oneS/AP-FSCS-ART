...
public class Simulation_Mutiply_same_region_7 {
	...
	double fail_Length; // 用于存储的是多个面积不同时的每个失效的面积的r
	ArrayList<ArrayList<Double>> mutiple; // 存放的是 多个失效域的坐标点

	
	int fail_number;
	public Simulation_Mutiply_same_region_7(ArrayList<Dimension> dim_list, double failrate, int a) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
		...
		this.fail_Length=sqrt(fail_totalsize / fail_number, dim_list.size());
	 ...
	 boolean fail1(ArrayList<Double> templist) {
		...
		//		  System.out.println(1);
				   if(ex(mutiple.get(i), templist,this.fail_Length)){
	...
	public int  run() {
		...
		ArrayList<Double> templist = new ArrayList<>();
		for (int j = 0; j < dim_list.size(); j++) {
			double temp = dim_list.get(j).getMin() + ((dim_list.get(j).getRange()-this.fail_Length) * new Random().nextDouble());		
			templist.add(temp);
		}
		mutiple.add(templist);
	...
	private boolean isCorrect(point testcase) {
		...
		for (int i = 0; i < mutiple.size(); i++) {
			...
			for (int j = 0; j < temp.size(); j++) {
				if ((testcase.point.get(j) < temp.get(j) || testcase.point.get(j) > temp.get(j) + this.fail_Length)) {
					// 测试用例落在了失效域，则返回false
...
//		}
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback5(list, failrate));
		}
		try {
			for(int i=1;i<=times;i++){
...
class ThreadWithCallback5 implements Callable{
	private ArrayList<Dimension> list;
	private double fail_rate;
	ThreadWithCallback5(ArrayList<Dimension> list,double fail_rate){
		this.list=list;
	...
	public Object call() throws Exception {
		...
		int temp=new Simulation_Mutiply_same_region_7(list,fail_rate,90).run();
		return temp;
	}
