...
public class My_Time2 {
	...
	public My_Time2(ArrayList<Dimension> dim_list,int count,	long startTime) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
					...
					//		System.out.println("是否有"+testRegion.contains(temp));
						//	ArrayList<Integer> index=new ArrayList<>();
							...
							for(int j=0;j<temp.point.point.size();j++){
								index.add((int) (temp.point.point.get(j)/single_length.get(j)));
							}
	...
	private void divide(Region region, LinkedList<Region> l, ArrayList<Region> tempRegion) {
		...
		for (int i = 0; i < result_value.size(); i++) {
			...
			//		System.out.println(region1.point.point.get(j)+"  "+single_length.get(j)+"  "+region1.point.point.get(j)/single_length.get(j));
					index.add((int) (region1.point.point.get(j)/single_length.get(j)));
				}
...
//	    list.add(z);
	//	list.add(u);
		...
		for (int i = 1; i <= times; i++) {
			...
			ArrayList<Long> sing=new My_Time2(list,20000,startTime).run();
		...
		try {
			write.createExcel("C:\\Users\\lenovo\\Desktop\\第一阶段数据\\生成N_10个测试用例时间_Method_2.xlsx", "_Method_2");
		} catch (Exception e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
		}
		write.writeExcel(F_meaure, "C:\\Users\\lenovo\\Desktop\\第一阶段数据\\生成N_10个测试用例时间_Method_2.xlsx","_Method_2");

		
		//		ArrayList result=new ArrayList<>();
//
//		product_struct(result, 1, 2, 10);
//		Region a=new Region();
//		a.length.add(1.0);
//		ArrayList<Integer> index=new ArrayList<>();
//		index.add(1);
...
//	
//		
//		SetStruct(a, index, 1, 2, result);
//		
//		Region b=getstruct(index, 1, 2, result);
//		System.out.println(b.length.get(0));
	}
