...
//	main m = new main();
	...
	public real_program4(ArrayList<Dimension> dim_list) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//						for(int i=0;i<dim_list.size();i++){
//							r.add(point);
//							map.add((int) (region.point.point.get(i)/single_length.get(i)));
//						}
////						System.out.println(map);
//						recursive(r, result_value, 0, new ArrayList<>());
//						
//						for(int i=0;i<result_value.size();i++){
//							ArrayList<Integer> a=new ArrayList<>();
//							a.addAll(map);
//							for(int j=0;j<dim_list.size();j++){
//								a.set(j, (int) (a.get(j)+result_value.get(i).get(j)));
//							}
//							try{
//								Region e=getstruct(a, 1, dim_list.size(), this.strcut);
//								if(e.testcase!=null){
//									close.add(e);
//								}
//							}catch (Exception e) {
...
//				System.out.println(getstruct(index, 1, dim_list.size(), this.strcut));
	...
	public static void main(String args[]) {
		...
		for (int i = 1; i <= times; i++) {
			...
			temp = new real_program4(list).run();
