...
public class real_program6 {
	...
	public real_program6(ArrayList<Dimension> dim_list) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//					divide(region, b, null);
	...
	//						+ (new Random().nextDouble() * b.get(j).length.get(i) * (1 - 2 * 0.3)));
					...
					for(int i=0;i<dim_list.size();i++){  //针对每个维度
					...
					for(int i=0;i<dim_list.size();i++){
				//		System.out.println(close.get(i)==0);
						if(close.get(i)==0){
							ArrayList<Integer> index=new ArrayList<>();
							index.addAll(close);
							index.set(i,index.get(i)+1);
							point test1=getstruct(index, 1, dim_list.size(), this.strcut).testcase;
							if(test1!=null){
								closelist.add(test1);
							}
							
						}else if(close.get(i)==maxindex[i]){
							ArrayList<Integer> index=new ArrayList<>();
							index.addAll(close);
							index.set(i,index.get(i)-1);
							point test1=getstruct(index, 1, dim_list.size(), this.strcut).testcase;
							...
							index.set(i,index.get(i)-2);
							test1=getstruct(index, 1, dim_list.size(), this.strcut).testcase;
							if(test1!=null){
								closelist.add(test1);
							}
						}
...
//				System.out.println(getstruct(index, 1, dim_list.size(), this.strcut));
	...
	public static void main(String args[]) {
		...
		for (int i = 1; i <= times; i++) {
			...
			temp = new real_program6(list).run();
