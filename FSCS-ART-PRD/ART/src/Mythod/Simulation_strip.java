...
public class Simulation_strip{
	...
	ArrayList<Double> fail_length=new ArrayList<>();

	
	public Simulation_strip(ArrayList<Dimension> dim_list,double failrate,double a) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
		...
		fail_length.add(sqrt(fail_totalsize/Math.pow(a, dim_list.size()-1), dim_list.size()));
		for(int i=1;i<dim_list.size();i++){
			fail_length.add(fail_length.get(0)*a);
		}
	...
	public int run() {
		...
		for (int i = 0; i < dim_list.size(); i++) {
			double temp = dim_list.get(i).getMin()
					+ ((dim_list.get(i).getRange() - fail_length.get(i)) * new Random().nextDouble());
			points.add(temp);
		}
...
//					
	...
	private boolean isCorrect(point testcase) {
		...
		for (int i = 0; i < points.size(); i++) {
			if (testcase.point.get(i) < points.get(i) || testcase.point.get(i) > points.get(i) + fail_length.get(i)) {
				jutice = true;
				break;
			}
		}
...
//		list.add(u);
		...
		for (int i = 1; i <= times; i++) {
			temp=new Simulation_strip(list,failrate,a).run();
			result.add(temp);
