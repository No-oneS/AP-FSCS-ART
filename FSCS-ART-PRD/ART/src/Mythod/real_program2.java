...
//	main m = new main();
	...
	public real_program2(ArrayList<Dimension> dim_list) {
		/*
		 * 求输入域面积，这是共存在步骤
		 */
...
//							+ (new Random().nextDouble() * b.get(j).length.get(i) * (1 - 2 * 0.3)));
			...
			//			System.out.println(testRegion.size());
						if(region.point.point.get(i)==dim_list.get(i).min&&region.length.get(i)<dim_list.get(i).getRange()){
					...
					//		System.out.println("是否有"+testRegion.contains(temp));
						...
						}else if(region.point.point.get(i)==dim_list.get(i).max-region.length.get(i)&&region.length.get(i)<dim_list.get(i).getRange()){
...
//			 return m.getResult("bessj", testcase.point.get(0),testcase.point.get(1));
		 return m.getResult("el2", testcase.point.get(0),testcase.point.get(1),testcase.point.get(2),testcase.point.get(3));
	...
	public static void main(String args[]) {
		...
		for (int i = 1; i <= times; i++) {
			...
			temp = new real_program2(list).run();
