...
public class FSCS_ctsr_real {
	...
	double fzb_rate = 0.9;//	
//	golden real = new golden();			// 初始化真实程序类	
	real_golden real = new real_golden();			// 初始化真实程序类

	public FSCS_ctsr_real(double[][]  bd, double failrate, int fail_number, boolean prodomain,
			double r,int f_k) {
...
//		}
		...
		while(true){
			if(!real.Produces_Error(testcase.list.get(0),testcase.list.get(1),testcase.list.get(2))){
...
//		System.out.println("2"+testcase2.list);
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int temp=new FSCS_ctsr_real(list, fail_rate, fail_number, prodomain, r,f_k).run();
			return temp;
		}
	}
	...
	public static void main(String args[]) {
		...
		int temp = 0;
		double failrate=real_Parameters.failure_rate_golden;
		int f_k=5;
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback(real_Parameters.bd_golden, failrate,fail_number, false, 0.3,f_k));	
		}
		long startTime = System.currentTimeMillis();
		try {
			for (int i = 1; i <= times; i++) {
	//			long startTime1 = System.currentTimeMillis();
	//			temp=new FSCS_ctsr_real(list, failrate, fail_number, false, 0.3,f_k).run();
...
//		System.out.println("Fm: " + sums / (double) times+"  且最后的Fart/Frt: "+sums / (double) times*failrate);// 平均每次使用的测试用例数
		...
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/real_software/FSCS_ctsr_real_data/FSCS_ctsr_real_golden.txt");
		try {
			FileWriter stream = new FileWriter(file2);
			// stream.write("sdfdsfdsfds");
			stream.write(real_Parameters.bd_golden.length+"D \t failrate:  "+failrate+" \t Fm:  "+sums / (double) times+"\t 且最后的Fart/Frt:  "+sums / (double) times*failrate+"\t Time :  "+(endTime - startTime) / (double)times+"\r\n\n");

			for (int j = 0; j < data.size(); j++) {
				stream.write(data.get(j) + "\r\n");
			}
			stream.close();
