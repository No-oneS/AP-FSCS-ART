...
public class FSCS_dt_bound_center_real_k_10_final_9___{
...
//	el2 real = new el2();			// 初始化真实程序类
	...
	public FSCS_dt_bound_center_real_k_10_final_9___(double[][]  bd, double failrate, int fail_number){
...
//		bound_center_rate = Math.pow(0.5,1.0/bd.length);
		bound_center_rate = 0.7;
		bound_center_rate = (1 - Math.pow( bound_center_rate, dimlist.size())) / Math.pow( bound_center_rate, dimlist.size());
...
//                	double dist=Min_Distance(c.get(i), e.get(j));
            ...
            center_num++;
//        	System.out.println("center_num ="+center_num);
        }else {
        	for (int i = 0; i < c.size(); i++) {
//            	System.out.println(" dimlist.size()="+dimlist.size());
                mindist = Double.MAX_VALUE;
...
//        	System.out.println("bound_num ="+bound_num);
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int[] temp=new FSCS_dt_bound_center_real_k_10_final_9___(list,fail_rate,fail_number).run();;
...
//			System.out.println("temp[2] ="+temp[2]);
	...
	public static void main(String args[]){
		...
		double M_Edge_Centre_sums = 0.0;
		int[] temp = new int[3];// 初始化测试用例落在失效域的使用的测试用例的个数
		double failrate= real_Parameters.failure_rate_el2;
		int fail_number = 25;		// 若为1，则表示block，大于1则表示point，若为负数则表示strip，strip时记得更新fzb
		ArrayList<Integer> result=new ArrayList<>();
		ArrayList<Double> result_M_Edge_Centre=new ArrayList<>();
...
//		}
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback(real_Parameters.bd_el2, failrate,fail_number));	
		}
...
//				System.out.println("future.take() = " + future.get() );
				result.add(temp[0]);
				result_M_Edge_Centre.add(temp[1]*1.0 / temp[2]);
				System.out.println("第"+i+"次试验F_Measure："+temp[0]+" and M(Edge:Centre) ="+temp[1]*1.0 / temp[2]);
				sums+=temp[0];
				M_Edge_Centre_sums += temp[1]*1.0 / temp[2];
			}
		} catch (Exception e) {
		...
		System.out.println("FSCS_dt_bound_center_real_k_10_final_9___ failrate ： "+ failrate +" ,fail_number "+fail_number+"\nFm: " + sums / (double) times+"  且最后的Fart/Frt: "+sums / (double) times*failrate+" M_Edge_Centre_ave :"+M_Edge_Centre_sums / (double) times);// 平均每次使用的测试用例数
		System.out.println("Time : "+(endTime - startTime) / (double)times );
		
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/real_software/FSCS_dt_bound_center_real_k_10_final_data/FSCS_dt_bound_center_real_k_10_final_9____el2.txt");
		try {
			FileWriter stream = new FileWriter(file2);
			// stream.write("sdfdsfdsfds");
			stream.write("real_Parameters :  el2 \t failrate:  "+ fail_number+"\t Fm:  "+sums / (double) times+"\t 且最后的Fart/Frt:  "+sums / (double) times*failrate+"\t\t   Time :  "+(endTime - startTime) / (double)times+"\r\n\n");

			for (int j = 0; j < result.size(); j++) {
				stream.write("Fm:  "+result.get(j) + "\rresult_M_Edge_Centre:"+result_M_Edge_Centre.get(j)+" \n");
			}
			stream.close();
