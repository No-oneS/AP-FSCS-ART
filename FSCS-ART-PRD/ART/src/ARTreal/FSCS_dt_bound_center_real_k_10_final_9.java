...
public class FSCS_dt_bound_center_real_k_10_final_9{
	...
	real_bessj0 real = new real_bessj0();			// 初始化真实程序类
	
//	bessj0 real = new bessj0();			// 初始化真实程序类
	...
	public FSCS_dt_bound_center_real_k_10_final_9(double[][]  bd, double failrate, int fail_number){
...
//                	ratio = ratio > (2 * tmp_min / Math.abs(dimlist.get(j).getMax() - dimlist.get(j).getMin())) ? ratio : (2 * tmp_min / Math.abs(dimlist.get(j).getMax() - dimlist.get(j).getMin()));   	
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int[] temp=new FSCS_dt_bound_center_real_k_10_final_9(list,fail_rate,fail_number).run();;
...
//			System.out.println("temp[2] ="+temp[2]);
	...
	public static void main(String args[]){
		...
		double failrate= real_Parameters.failure_rate_bessj0;
...
//		}
		...
		for (int i=1; i <= times; i++) {
			 cService.submit(new ThreadWithCallback(real_Parameters.bd_bessj0, failrate,fail_number));	
		}
...
//				System.out.println("future.take() = " + future.get() );
		...
		System.out.println("FSCS_dt_bound_center_real_k_10_final_9 failrate ： "+ failrate +" ,fail_number "+fail_number+"\nFm: " + sums / (double) times+"  且最后的Fart/Frt: "+sums / (double) times*failrate);// 平均每次使用的测试用例数
		System.out.println("Time : "+(endTime - startTime) / (double)times );
		
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/real_software/FSCS_dt_bound_center_real_k_10_final_data/FSCS_dt_bound_center_real_k_10_final_9_bessj0.txt");
		try {
			FileWriter stream = new FileWriter(file2);
			// stream.write("sdfdsfdsfds");
			stream.write("real_Parameters :  bessj0 \t failrate:  "+ fail_number+"\t Fm:  "+sums / (double) times+"\t 且最后的Fart/Frt:  "+sums / (double) times*failrate+"\t\t   Time :  "+(endTime - startTime) / (double)times+"\r\n\n");
