...
public class FSCS_dt_bound_center_real_k_10_final_9_not_ratio_and_distance{
...
//	el2 real = new el2();			// 初始化真实程序类
	...
	public FSCS_dt_bound_center_real_k_10_final_9_not_ratio_and_distance(double[][]  bd, double failrate, int fail_number){
...
//            	ratio = ratio > (2 * tmp_min / Math.abs(dimlist.get(j).getMax() - dimlist.get(j).getMin())) ? ratio : (2 * tmp_min / Math.abs(dimlist.get(j).getMax() - dimlist.get(j).getMin()));   	
	...
	static class ThreadWithCallback implements Callable{
		...
		public Object call() throws Exception {
			...
			int temp=new FSCS_dt_bound_center_real_k_10_final_9_not_ratio_and_distance(list,fail_rate,fail_number).run();;
			return temp;
		}
	}
...
//				System.out.println("future.take() = " + future.get() );
		...
		System.out.println("FSCS_dt_bound_center_real_k_10_final_9_not_ratio_and_distance failrate ： "+ failrate +" ,fail_number "+fail_number+"\nFm: " + sums / (double) times+"  且最后的Fart/Frt: "+sums / (double) times*failrate);// 平均每次使用的测试用例数
		System.out.println("Time : "+(endTime - startTime) / (double)times );
		
		java.io.File file2 = new java.io.File("FSCS_dt_bound_center_data/real_software/FSCS_dt_bound_center_real_k_10_final_data/FSCS_dt_bound_center_real_k_10_final_9_not_ratio_and_distance_el2.txt");
		try {
